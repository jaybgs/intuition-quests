...
interface EditSpaceVisualsModalProps {
    space: Space;
    onClose: () => void;
    onUpdate: (updatedSpace: Space) => void;
}

export function EditSpaceVisualsModal({ space, onClose, onUpdate }: EditSpaceVisualsModalProps) {
    const [formData, setFormData] = useState({
        name: space.name,
        description: space.description,
        logo: space.logo || '',
        coverPhoto: space.coverPhoto || '',
        twitterUrl: space.twitterUrl || '',
    });
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'cover') => {
        ...
        try {
            // Use space ID as folder path to organize images
            const path = `space-${space.id}`;
            const url = await storageService.uploadImage(file, 'space-images', path);

            if (url) {
                // Map 'cover' type back to 'coverPhoto' field
                const fieldName = type === 'cover' ? 'coverPhoto' : 'logo';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (formData.name !== space.name) updates.name = formData.name;
            if (formData.description !== space.description) updates.description = formData.description;
            if (formData.logo !== space.logo) updates.logo = formData.logo || undefined;
            if (formData.coverPhoto !== space.coverPhoto) updates.coverPhoto = formData.coverPhoto || undefined;
            if (formData.twitterUrl !== space.twitterUrl) updates.twitterUrl = formData.twitterUrl;
            ...
            // We use the spaceService which handles fallback/Supabase internally
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <h3 style={{ margin: 0, color: 'white', fontSize: '18px', fontWeight: 600 }}>Edit Space Visuals</h3>
                    <button
                        onClick={onClose}
                        style={{
                ...
                >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <label style={{ color: '#cbd5e1', fontSize: '14px', fontWeight: 500 }}>Name</label>
                        <input
                            type="text"
                            name="name"
                            value={formData.name}
                    ...
                    {/* Twitter URL */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <label style={{ color: '#cbd5e1', fontSize: '14px', fontWeight: 500 }}>Twitter URL</label>
                        <input
                            type="text"
                            name="twitterUrl"
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        <div style={{
                            ...
                            {formData.coverPhoto ? (
                                <img src={formData.coverPhoto} alt="Cover Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            ) : (
