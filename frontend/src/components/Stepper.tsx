import React, { useState, Children, useRef, useLayoutEffect, ButtonHTMLAttributes } from 'react';
...
interface StepperProps {
    ...
    onBeforeNext?: (nextStep: number) => boolean | Promise<boolean>;
...
export default function Stepper({
    ...
    const handleNext = async () => {
        if (!isLastStep) {
            ...
            if (onBeforeNext) {
                const canProceed = await onBeforeNext(nextStep);
    ...
    const handleComplete = async () => {
        ...
        if (onBeforeNext) {
            const canProceed = await onBeforeNext(nextStep);
            console.log('Proceeding to complete?', canProceed);
            if (!canProceed) return;
        }
                                ...
                                {renderStepIndicator ? (
                                    renderStepIndicator({
                                        ...
                                        onStepClick: (clicked) => {
                                            ...
                                            // Navigation logic for direct clicks
                                    ...
                                    <StepIndicator
                                        ...
                                        onClickStep={(clicked: number) => {
                                            if (disableStepIndicators) return;
                                            // Block forward jumps if validation required
                                            if (onBeforeNext && clicked > currentStep) return;
